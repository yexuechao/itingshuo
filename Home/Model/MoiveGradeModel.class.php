...
class MovieGradeModel extends Model{
    ...
    function moveToHist($emotion_id,$movie_id,$segment_id,$eval_yinzhun,$eval_qinggan,$eval_zhongyin,$eval_yusu,$eval_jiezou,$eval_yudiao,$eval_total){
        ...
        //把这个表的数据插入历史表
        $ress=$this->where("emotion_id=$emotion_id and movie_id=$movie_id and segment_id=$segment_id and user_id=".$_SESSION["user"]['user_id'])->find();
    ...
    function getClassGrade($class_id,$emotion_id,$movie_id,$segment_id){
        $res=$this->where("class_id=$class_id and emotion_id=$emotion_id and movie_id=$movie_id and segment_id=$segment_id")->select();
        return $res;
    }

    function getPersonGrade($user_id,$emotion_id,$movie_id,$segment_id){
        $res=$this->where("user_id=$user_id and emotion_id=$emotion_id and movie_id=$movie_id and segment_id=$segment_id")->select();
        //获取班级详细信息
        $model=new Model();
        $class_info=$sql="select c.class_name,i.institute_name from its_class as c,its_institute as i where c.institute_id=i.institute_id and c.class_id=".$res[0]["class_id"];
